...
export default class ThreeEngine {
  constructor(domRoot, miniDom) {
    ...
    this.minPolarAngle = Math.PI / 3; //最小限制角度
    this.maxPolarAngle = Math.PI / 3; //最大限制角度
    this.scene = new THREE.Scene();
    this.camera = null;    // 主相机
    this.mini_camera = null;
    this.renderer = null;  // 渲染器
    this.controls = null;  // 控制器
    this.lastTarX = null;  // 焦点坐标
    this.lastTarZ = null;
  ...
  _init() {
    // 画布尺寸
    const width = this.domRoot.offsetWidth;
    const height = this.domRoot.offsetHeight;
  ...
  _animate() {
    ...
    if (this.ndRender) {
      ...
      if (this.lastTarX !== tarX || this.lastTarZ !== tarZ) {
        this.lastTarX = tarX;
        this.lastTarZ = tarZ;
        initMap.call(this, tarX, tarZ);
      }
  ...
  _onResize() {
    const w = this.domRoot.offsetWidth;
    const h = this.domRoot.offsetHeight;
